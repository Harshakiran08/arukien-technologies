import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Mail, MapPin, Phone } from "lucide-react";
import { motion } from "motion/react";

const CONTACT_INFO = [
  {
    icon: MapPin,
    label: "Address",
    value: "123 Tech Park, Bangalore, Karnataka, India — 560001",
  },
  { icon: Phone, label: "Phone", value: "+91 98765 43210" },
  { icon: Mail, label: "Email", value: "info@arukientechnologies.com" },
];

// WhatsApp SVG Icon
function WhatsAppIcon({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="currentColor"
      className={className}
      role="presentation"
      aria-hidden="true"
    >
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
    </svg>
  );
}

export function ContactSection() {
  return (
    <section
      id="contact"
      className="py-24 bg-muted/20"
      data-ocid="contact.section"
    >
      <div className="container mx-auto px-6 lg:px-8">
        {/* Header */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
          className="text-center mb-16"
        >
          <Badge
            variant="secondary"
            className="mb-4 font-body text-xs tracking-widest uppercase px-4 py-1"
          >
            Get In Touch
          </Badge>
          <h2 className="font-display font-bold text-4xl md:text-5xl text-foreground mb-4">
            Contact Us
          </h2>
          <p className="font-body text-muted-foreground text-lg max-w-xl mx-auto">
            Ready to transform your enterprise? Reach us instantly on WhatsApp —
            our team is standing by to help.
          </p>
        </motion.div>

        <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 max-w-5xl mx-auto">
          {/* Contact info */}
          <motion.div
            initial={{ opacity: 0, x: -24 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="lg:col-span-2 flex flex-col gap-6"
          >
            <div className="p-6 rounded-2xl bg-card border border-border shadow-subtle">
              <h3 className="font-display font-semibold text-foreground text-lg mb-5">
                Reach Us Directly
              </h3>
              <div className="space-y-5">
                {CONTACT_INFO.map(({ icon: Icon, label, value }) => (
                  <div
                    key={label}
                    className="flex items-start gap-3"
                    data-ocid={`contact.info.${label.toLowerCase()}`}
                  >
                    <div className="w-9 h-9 rounded-lg bg-primary/10 border border-primary/20 flex items-center justify-center shrink-0 mt-0.5">
                      <Icon size={15} className="text-accent-bright" />
                    </div>
                    <div>
                      <p className="font-body text-xs text-muted-foreground tracking-wide">
                        {label}
                      </p>
                      <p className="font-body text-sm text-foreground mt-0.5 break-words">
                        {value}
                      </p>
                    </div>
                  </div>
                ))}
              </div>
            </div>

            {/* Book a Demo card */}
            <div className="p-6 rounded-2xl bg-primary/10 border border-primary/20">
              <h4 className="font-display font-semibold text-foreground text-base mb-2">
                Book a Demo
              </h4>
              <p className="font-body text-sm text-muted-foreground mb-4 leading-relaxed">
                See Arukien Technologies in action. Schedule a personalized
                30-minute demo with our solutions team.
              </p>
              <a
                href="https://wa.me/8792727104"
                target="_blank"
                rel="noopener noreferrer"
                data-ocid="contact.book_demo_button"
              >
                <Button
                  type="button"
                  variant="secondary"
                  size="sm"
                  className="w-full font-body font-medium gap-2"
                >
                  <WhatsAppIcon className="w-4 h-4" />
                  Chat to Schedule
                </Button>
              </a>
            </div>
          </motion.div>

          {/* WhatsApp CTA Panel */}
          <motion.div
            initial={{ opacity: 0, x: 24 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.1 }}
            className="lg:col-span-3"
          >
            <div className="p-8 rounded-2xl bg-card border border-border shadow-subtle flex flex-col items-center justify-center text-center gap-7 min-h-[360px]">
              {/* WhatsApp icon */}
              <div
                className="w-20 h-20 rounded-full flex items-center justify-center shadow-elevated"
                style={{
                  background:
                    "linear-gradient(135deg, #25D366 0%, #128C7E 100%)",
                }}
              >
                <WhatsAppIcon className="w-10 h-10 text-white" />
              </div>

              <div>
                <h3 className="font-display font-semibold text-foreground text-2xl mb-2">
                  Chat with Us on WhatsApp
                </h3>
                <p className="font-body text-muted-foreground text-base leading-relaxed max-w-sm mx-auto">
                  Skip the forms — connect with our team directly. We're ready
                  to discuss your project, answer questions, and get started
                  right away.
                </p>
              </div>

              {/* Reply-time indicator */}
              <div
                className="flex items-center gap-2 px-4 py-2 rounded-full border text-sm font-medium"
                style={{
                  background: "#f0fdf4",
                  borderColor: "#86efac",
                  color: "#15803d",
                }}
              >
                <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                We typically reply within a few hours
              </div>

              {/* CTA Button */}
              <a
                href="https://wa.me/8792727104"
                target="_blank"
                rel="noopener noreferrer"
                data-ocid="contact.whatsapp_button"
                className="w-full max-w-xs"
              >
                <Button
                  type="button"
                  size="lg"
                  className="w-full gap-3 font-body font-semibold text-base py-6 rounded-xl text-white shadow-lg hover:shadow-xl transition-smooth hover:-translate-y-0.5"
                  style={{
                    background:
                      "linear-gradient(135deg, #25D366 0%, #128C7E 100%)",
                  }}
                >
                  <WhatsAppIcon className="w-5 h-5 text-white" />
                  Start WhatsApp Chat
                </Button>
              </a>

              <p className="text-xs text-muted-foreground">
                Opens WhatsApp in a new tab •{" "}
                <span className="font-medium text-foreground">
                  +91 87927 27104
                </span>
              </p>
            </div>
          </motion.div>
        </div>
      </div>
    </section>
  );
}
